...
import merge from 'sinoUI/src/utils/merge';
...
const radioRowSelection = function(states, row, selected) {
    let changed = false;
    const selection = states.selection;
    const index = selection.indexOf(row);
    if(selection.length > 0){
        selection.splice(0,1,row);
        if (typeof selected === 'undefined') {
            if (index !== -1) {
                selection.splice(index, 1);
                changed = true;
...
TableStore.prototype.mutations = {
  ...
  rowSelectedChanged(states, row) {
    ...
    rowSelectedChangedRadio(states, row) {
        const changed = radioRowSelection(states, row);
        const selection = states.selection;

        if (changed) {
            const table = this.table;
            table.$emit('radioSelectionChange', selection);
            table.$emit('radioSelect', selection, row);
        }
    },
