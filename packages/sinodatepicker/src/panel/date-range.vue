<template>
  <transition name="sino-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      :style="{ width: width + 'px' }"
      class="sino-picker-panel sino-date-range-picker"
      :class="[{
        'has-sidebar': $slots.sidebar || shortcuts,
        'has-time': showTime
      }, popperClass]">
      <div class="sino-picker-panel-body-wrapper">
        <slot name="sidebar" class="sino-picker-panel-sidebar"></slot>
        <div class="sino-picker-panel-sidebar" v-if="shortcuts">
          <button
            type="button"
            class="sino-picker-panel-shortcut"
            v-for="shortcut in shortcuts"
            @click="handleShortcutClick(shortcut)">{{shortcut.text}}</button>
        </div>
        <div class="sino-picker-panel-body">
          <div class="sino-date-range-picker-time-header" v-if="showTime">
            <span class="sino-date-range-picker-editors-wrap">
              <span class="sino-date-range-picker-time-picker-wrap">
                <sino-input
                  size="small"
                  ref="minInput"
                  :placeholder="t('el.datepicker.startDate')"
                  class="sino-date-range-picker-editor"
                  :value="minVisibleDate"
                  @input.native="handleDateInput($event, 'min')"
                  @change.native="handleDateChange($event, 'min')"></sino-input>
              </span>
              <span class="sino-date-range-picker-time-picker-wrap">
                <sino-input
                  size="small"
                  ...
                  @change.native="handleTimeChange($event, 'min')"></sino-input>
                <time-picker
            ...
            <span class="sino-icon-arrow-right"></span>
            <span class="sino-date-range-picker-editors-wrap is-right">
              <span class="sino-date-range-picker-time-picker-wrap">
                <sino-input
                  size="small"
                  :placeholder="t('e.datepicker.endDate')"
                  class="sino-date-range-picker-editor"
                  :value="maxVisibleDate"
                  :readonly="!minDate"
                  @input.native="handleDateInput($event, 'max')"
                  @change.native="handleDateChange($event, 'max')"></sino-input>
              </span>
              <span class="sino-date-range-picker-time-picker-wrap">
                <sino-input
                  size="small"
                  ...
                  :readonly="!minDate"
                  @change.native="handleTimeChange($event, 'max')"></sino-input>
                <time-picker
          ...
          <div class="sino-picker-panel-content sino-date-range-picker-content is-left">
            <div class="sino-date-range-picker-header">
              <button
                type="button"
                @click="prevYear"
                class="sino-picker-panel-icon-btn sino-icon-d-arrow-left"></button>
              <button
                type="button"
                @click="prevMonth"
                class="sino-picker-panel-icon-btn sino-icon-arrow-left"></button>
              <div>{{ leftLabel }}</div>
            </div>
            <date-table
          ...
          <div class="sino-picker-panel-content sino-date-range-picker-content is-right">
            <div class="sino-date-range-picker-header">
              <button
                type="button"
                @click="nextYear"
                class="sino-picker-panel-icon-btn sino-icon-d-arrow-right"></button>
              <button
                type="button"
                @click="nextMonth"
                class="sino-picker-panel-icon-btn sino-icon-arrow-right"></button>
              <div>{{ rightLabel }}</div>
            </div>
            <date-table
              selection-mode="range"
      ...
      <div class="sino-picker-panel-footer" v-if="showTime">
        <a
          class="sino-picker-panel-link-btn"
          @click="handleClear">{{ t('el.datepicker.clear') }}</a>
        <button
          type="button"
          class="sino-picker-panel-btn"
          @click="handleConfirm()"
...
<script type="text/babel">
  ...
  export default {
    ...
    components: { TimePicker, DateTable, sinoInput }
  };
</script>
