<template>
  <transition name="sino-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      ...
      class="sino-picker-panel sino-date-picker"
      :class="[{
      ...
      <div class="sino-picker-panel-body-wrapper">
        ...
        <div class="sino-picker-panel-body">
          <div class="sino-date-picker-time-header" v-if="showTime">
            <span class="sino-date-picker-editor-wrap">
              <sino-input
                :placeholder="t('el.datepicker.selectDate')"
                :value="visibleDate"
                size="small"
                @change.native="visibleDate = $event.target.value"></sino-input>
            </span>
            <span class="sino-date-picker-editor-wrap">
              <sino-input
                ref="input"
                ...
                :value="visibleTime"
                size="small"
                @change.native="visibleTime = $event.target.value" ></sino-input>
              <time-picker
                ref="timepicker"
                :date="date"
          ...
          <div class="sino-date-picker-header" v-show="currentView !== 'time'">
            <button
              type="button"
              @click="prevYear"
              class="sino-picker-panel-icon-btn iconfont sino-icon44">
            </button>
            <button
              type="button"
              @click="prevMonth"
              v-show="currentView === 'date'"
              class="sino-picker-panel-icon-btn iconfont sino-left">
            </button>
            <span
              @click="showYearPicker"
              class="sino-date-picker-header-label">{{ yearLabel }}</span>
            <span
              @click="showMonthPicker"
              v-show="currentView === 'date'"
              class="sino-date-picker-header-label"
              :class="{ active: currentView === 'month' }">{{t(`el.datepicker.month${ month + 1 }`)}}</span>
            <button
              type="button"
              @click="nextMonth"
              v-show="currentView === 'date'"
              class="sino-picker-panel-icon-btn iconfont sino-xiangyou">
            </button>
            <button
              type="button"
              @click="nextYear"
              class="sino-picker-panel-icon-btn iconfont sino-icon53">
            </button>
          </div>

          <div class="sino-picker-panel-content">
            <date-table
              v-show="currentView === 'date'"
              @pick="handleDatePick"
              :year="year"
      ...
      <div
        class="sino-picker-panel-footer"
...
<script type="text/babel">
  ...
  export default {
    ...
    components: {
      TimePicker, YearTable, MonthTable, DateTable, sinoInput
    },
